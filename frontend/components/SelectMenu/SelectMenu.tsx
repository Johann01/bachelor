...
function SelectMenu() {
  ...
  const handleOnChange = (selectedFeaturesNew: Feature[]) => {
    var difference = selectedFeatures.filter(
      (x) => !selectedFeaturesNew.includes(x)
    );
    if (difference.length > 0) {
      updateFeature({ ...difference[0], activated: false });
    }

    selectedFeaturesNew.forEach((feature) =>
      updateFeature({ ...feature, activated: true })
    );
  };
